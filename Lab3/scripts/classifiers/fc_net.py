    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            if self.normalization == 'batchnorm':
                ...
                caches.append(('bn', fc_cache, bn_cache, relu_cache))
            else:
                x, cache = affine_relu_forward(x, W, b)
                caches.append(('normal', cache))
            if self.use_dropout:
                x, dc = dropout_forward(x, self.dropout_param)
                drop_caches.append(dc)
        ...
        dx, dW, db = affine_backward(dscores, last_cache)
        ...
        for i in range(self.num_layers - 2, -1, -1):
            ...
            cache = caches[i]
            if cache[0] == 'bn':
                _, fc_cache, bn_cache, relu_cache = cache
