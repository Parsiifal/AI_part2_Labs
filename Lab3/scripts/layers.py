...
def batchnorm_backward(dout, cache):
    ...
    dmu = np.sum(dx_hat / -np.sqrt(var + eps), axis=0) + dvar * np.sum(-2*(x - mu), axis=0) / N
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    region = x[n,c,i*stride:i*stride+ph,j*stride:j*stride+pw]
                    mask = (region == np.max(region))
                    dx[n,c,i*stride:i*stride+ph,j*stride:j*stride+pw] += mask * dout[n,c,i,j]
